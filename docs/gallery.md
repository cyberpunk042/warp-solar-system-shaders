...
> consuming the previous process's real output, each shown whole. The earlier versions were retired for
> cutting corners (idealised shapes generated by index rather than folding the real strand); they'll
> return one at a time, verified — the ~1663 beads coiling ~6 per turn into the 30 nm fibre, then folding
> into the two chromatids of the metaphase chromosome with its centromere and telomere caps.
