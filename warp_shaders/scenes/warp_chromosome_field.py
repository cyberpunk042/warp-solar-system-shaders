...
def _ensure(device):
    ...
    if _ba is None:
        _ba = wp.array(_TL.tel_a, dtype=wp.vec3, device=device)
        _bb = wp.array(_TL.tel_b, dtype=wp.vec3, device=device)
        _fa = wp.array(_CH.chr_a, dtype=wp.vec3, device=device)
        _fb = wp.array(_CH.chr_b, dtype=wp.vec3, device=device)
        _a_col = wp.array(_CH.a_col, dtype=wp.vec3, device=device)
        _b_col = wp.array(_CH.b_col, dtype=wp.vec3, device=device)
